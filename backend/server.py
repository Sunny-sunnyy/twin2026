...
bedrock_client = boto3.client(
    ...
    region_name=os.getenv("DEFAULT_AWS_REGION", os.getenv("AWS_REGION", "ap-southeast-1"))
)
    ...
    try:
        ...
        if error_code == 'ValidationException':
            ...
            raise HTTPException(
                status_code=429,
                detail=(
                    "Bedrock quota exceeded for this model or inference profile. "
                    "Check Service Quotas in the Bedrock region you are calling and "
                    "try a global cross-region inference profile."
                ),
            )
        else:
